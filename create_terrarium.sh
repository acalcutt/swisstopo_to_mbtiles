...
INPUT_DIR=./download
OUTPUT_DIR=./output
vrtfile=${OUTPUT_DIR}/swissalti3d_terrarium0-16.vrt
mbtiles=${OUTPUT_DIR}/swissalti3d_terrarium0-16.mbtiles
vrtfile2=${OUTPUT_DIR}/swissalti3d_terrarium0-16_warp.vrt
...
rio rgbify -e terrarium --min-z 0 --max-z 16 -j 24 --format png ${vrtfile2} ${mbtiles}

#sqlite3 ${mbtiles} 'CREATE UNIQUE INDEX tile_index on tiles (zoom_level, tile_column, tile_row);'
sqlite3 ${mbtiles} 'UPDATE metadata SET value = "swissalti3d_terrarium_z0-Z16_webp" WHERE name = "name" AND value = "";'
